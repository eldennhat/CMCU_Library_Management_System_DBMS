...
from database.logger_service import logger #import 'logger' from logger_service
...
def register_new_user(username, password, role):
    ...
    if conn is None:
        ...
        logger.insert_log(username, "Register Error", "Database Connection Failed", {"target_user": username})
        return "DB_ERROR"
    cursor = conn.cursor()
    ...
    try:
        ...
        logger.insert_log(username, "Register", f"Registered new user: {username}", {"role": role})
        ...
        if '2627' in error_code or '2601' in error_code:
            ...
            logger.insert_log(username, "Register Failed", f"Username '{username}' already exists", {"error_code": error_code})
            return "USERNAME_EXISTS"
        else:
            # Các lỗi SQL khác
            print(f"Lỗi truy vấn SQL (register): {e}")
            logger.insert_log(username, "Register Error", f"Database Error: {e}", {"target_user": username})
            return "DB_ERROR"
...
def update_staff_details(staff_id, full_name, phone, start_time, end_time, role):
    ...
    try:
        ...
        # Add log for update
        logger.insert_log("Admin", "Update Staff", f"Updated staff ID {staff_id}", 
                          {"staff_id": staff_id, "new_fullname": full_name, "new_role": role})
        ...
        # Add log for error
        logger.insert_log("Admin", "Update Staff Error", f"Failed to update staff ID {staff_id}: {e}", {"staff_id": staff_id})
...
def delete_staff_and_account(staff_id):
    ...
    try:
        ...
        # Add log for delete
        logger.insert_log("Admin", "Delete Staff", f"Deleted staff ID {staff_id}", {"staff_id": staff_id})
        ...
        # Add log for error
        logger.insert_log("Admin", "Delete Staff Error", f"Failed to delete staff ID {staff_id}: {e}", {"staff_id": staff_id})
...
def create_staff_and_account(staff_data, account_data):
    ...
    try:
        ...
        conn.commit()
        logger.insert_log(username, "Create Staff", f"Created staff: {username} (ID: {new_staff_id})", {"role": role})
